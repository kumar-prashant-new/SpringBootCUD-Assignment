...
public class CustomerService {
	...
	String[] successMsg = { "SUCCESS", "N/A", "Operation was success" }; // For
																			// Status
	...
	public CustomerResponse createCustomer(Customer customer) {
		String[] errMsg = {
				"FAILED",
				customer.getId(),
				"Existing User with this id or Email / Please register with some other id and email than: "
						+ customer.getId() + " and " + customer.getEmail() };
		boolean itemExist = false;
		if (customerList.isEmpty()) {
			customerList.add(customer);
			return new CustomerResponse("User Successfully Created",
					customer.getId(), successMsg);
		}
		...
		if (itemExist == false) {
			...
			return new CustomerResponse("Updation Failed", customer.getId(),
					errMsg);
		}
		...
		for (int i = 0; i < customerList.size(); i++) {
			...
			if (cust.getId().equals(id)) {
				...
				return new CustomerResponse(
						"Customer deactivated successfully", id, successMsg);
			}
		}
	...
	public CustomerResponse updateCustomer(Customer customer, String id) {
		...
		for (int i = 0; i < customerList.size(); i++) {
			...
			if (cust.getId().equals(id)) {
				...
				return new CustomerResponse("Customer updated successfully",
						id, successMsg);
			}
		}
		return new CustomerResponse("Updation Failed", id, errMsg);
